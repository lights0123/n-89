...
extern "C" {
    ...
    #[no_mangle]
    fn FlashReadByte(addr: uint32_t) -> uint8_t;
    #[no_mangle]
    fn FlashReadWord(addr: uint32_t) -> uint16_t;
    #[no_mangle]
    fn FlashReadLong(addr: uint32_t) -> uint32_t;
    #[no_mangle]
    ...
    // ---
    #[no_mangle]
    fn io3_get_byte(addr: uint32_t) -> uint8_t;
    #[no_mangle]
    fn io3_get_word(addr: uint32_t) -> uint16_t;
    #[no_mangle]
    fn io3_get_long(addr: uint32_t) -> uint32_t;
    #[no_mangle]
    fn io3_put_long(addr: uint32_t, arg: uint32_t);
    #[no_mangle]
    fn io3_put_word(addr: uint32_t, arg: uint16_t);
    #[no_mangle]
    fn io3_put_byte(addr: uint32_t, arg: uint8_t);
}
...
#[no_mangle]
pub unsafe extern "C" fn ti89t_mem_init() -> cty::c_int {
    // set mappers
    mem_get_byte_ptr =
        Some(ti89t_get_byte as unsafe extern "C" fn(_: uint32_t) -> uint8_t);
    mem_get_word_ptr =
        Some(ti89t_get_word as unsafe extern "C" fn(_: uint32_t) -> uint16_t);
    mem_get_long_ptr =
        Some(ti89t_get_long as unsafe extern "C" fn(_: uint32_t) -> uint32_t);
    mem_put_byte_ptr =
        Some(ti89t_put_byte as
                 unsafe extern "C" fn(_: uint32_t, _: uint8_t) -> ());
    mem_put_word_ptr =
        Some(ti89t_put_word as
                 unsafe extern "C" fn(_: uint32_t, _: uint16_t) -> ());
    mem_put_long_ptr =
        Some(ti89t_put_long as
                 unsafe extern "C" fn(_: uint32_t, _: uint32_t) -> ());
    mem_get_real_addr_ptr =
        Some(ti89t_get_real_addr as
                 unsafe extern "C" fn(_: uint32_t) -> *mut uint8_t);
    return 0i32;
}
#[no_mangle]
pub unsafe extern "C" fn ti89t_get_real_addr(mut adr: uint32_t)
 -> *mut uint8_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x3ffffi32 as cty::c_uint ||
           adr >= 0x200000i32 as cty::c_uint &&
               adr <= 0x23ffffi32 as cty::c_uint ||
           adr >= 0x400000i32 as cty::c_uint &&
               adr <= 0x43ffffi32 as cty::c_uint {
        return tihw.ram.offset((adr & 0x3ffffi32 as cty::c_uint) as isize)
    } else {
        // FLASH access
        if adr >= 0x800000i32 as cty::c_uint &&
               adr <= 0xbfffffi32 as cty::c_uint {
            return tihw.rom.offset((adr &
                                        (4i32 * (1024i32 * 1024i32) - 1i32) as
                                            cty::c_uint) as isize)
        } else {
            // memory-mapped I/O
            if adr >= 0x600000i32 as cty::c_uint &&
                ...
                if adr >= 0x700000i32 as cty::c_uint &&
                       adr <= (0x700000i32 + (256i32 - 1i32)) as cty::c_uint
                   {
                    ...
                    if adr >= 0x710000i32 as cty::c_uint &&
                           adr <=
                               (0x710000i32 + (256i32 - 1i32)) as cty::c_uint
                       {
                        return tihw.io3.offset((adr &
                                                    (256i32 - 1i32) as
                                                        cty::c_uint) as
                                                   isize)
                    }
...
#[no_mangle]
pub unsafe extern "C" fn ti89t_get_long(mut adr: uint32_t) -> uint32_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x3ffffi32 as cty::c_uint ||
           adr >= 0x200000i32 as cty::c_uint &&
               adr <= 0x23ffffi32 as cty::c_uint ||
           adr >= 0x400000i32 as cty::c_uint &&
               adr <= 0x43ffffi32 as cty::c_uint {
        return ((((*tihw.ram.offset((adr & 0x3ffffi32 as cty::c_uint) as
                                        isize) as cty::c_int) << 8i32 |
                      *tihw.ram.offset((adr.wrapping_add(1i32 as cty::c_uint)
                                            & 0x3ffffi32 as cty::c_uint) as
                                           isize) as cty::c_int) as uint16_t
                     as cty::c_int) << 16i32 |
                    ((*tihw.ram.offset((adr.wrapping_add(2i32 as cty::c_uint)
                         ...
                         *tihw.ram.offset((adr.wrapping_add(2i32 as
                                                                cty::c_uint).wrapping_add(1i32
                                                                                               as
                                                                                               cty::c_uint)
                                               & 0x3ffffi32 as cty::c_uint)
                                              as isize) as cty::c_int) as
                        uint16_t as cty::c_int) as uint32_t
    } else {
        // FLASH access
        if adr >= 0x800000i32 as cty::c_uint &&
               adr <= 0xbfffffi32 as cty::c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn ti89t_get_word(mut adr: uint32_t) -> uint16_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x3ffffi32 as cty::c_uint ||
           adr >= 0x200000i32 as cty::c_uint &&
               adr <= 0x23ffffi32 as cty::c_uint ||
...
#[no_mangle]
pub unsafe extern "C" fn ti89t_get_byte(mut adr: uint32_t) -> uint8_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x3ffffi32 as cty::c_uint ||
           adr >= 0x200000i32 as cty::c_uint &&
               adr <= 0x23ffffi32 as cty::c_uint ||
...
#[no_mangle]
pub unsafe extern "C" fn ti89t_put_long(mut adr: uint32_t,
                                        mut arg: uint32_t) {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x3ffffi32 as cty::c_uint ||
           adr >= 0x200000i32 as cty::c_uint &&
           ...
           adr >= 0x400000i32 as cty::c_uint &&
               adr <= 0x43ffffi32 as cty::c_uint {
        *tihw.ram.offset((adr & 0x3ffffi32 as cty::c_uint) as isize) =
            ((arg >> 16i32) as uint16_t as cty::c_int >> 8i32) as uint8_t;
        *tihw.ram.offset((adr.wrapping_add(1i32 as cty::c_uint) &
                              0x3ffffi32 as cty::c_uint) as isize) =
        ...
        *tihw.ram.offset((adr.wrapping_add(2i32 as
            ...
            ((arg & 0xffffi32 as cty::c_uint) as uint16_t as cty::c_int &
                 0xffi32) as uint8_t
    } else if adr >= 0x800000i32 as cty::c_uint &&
                  adr <= 0xbfffffi32 as cty::c_uint {
    ...
    } else if adr >= 0x710000i32 as cty::c_uint &&
                  adr <= (0x710000i32 + (256i32 - 1i32)) as cty::c_uint {
        io3_put_long(adr, arg);
    };
}
#[no_mangle]
pub unsafe extern "C" fn ti89t_put_word(mut adr: uint32_t,
                                        mut arg: uint16_t) {
    // FLASH access
    // memory-mapped I/O
    // memory-mapped I/O (hw2)
    ...
    if adr >= 0i32 as cty::c_uint && adr <= 0x3ffffi32 as cty::c_uint ||
           ...
           adr >= 0x400000i32 as cty::c_uint &&
               adr <= 0x43ffffi32 as cty::c_uint {
        *tihw.ram.offset((adr & 0x3ffffi32 as cty::c_uint) as isize) =
            (arg as cty::c_int >> 8i32) as uint8_t;
        *tihw.ram.offset((adr.wrapping_add(1i32 as cty::c_uint) &
                              0x3ffffi32 as cty::c_uint) as isize) =
...
#[no_mangle]
pub unsafe extern "C" fn ti89t_put_byte(mut adr: uint32_t, mut arg: uint8_t) {
    // FLASH access
    // memory-mapped I/O
    // memory-mapped I/O (hw2)
    // memory-mapped I/O (hw3)
    ...
    if adr >= 0i32 as cty::c_uint && adr <= 0x3ffffi32 as cty::c_uint ||
           ...
           adr >= 0x400000i32 as cty::c_uint &&
               adr <= 0x43ffffi32 as cty::c_uint {
        *tihw.ram.offset((adr & 0x3ffffi32 as cty::c_uint) as isize) = arg
    } else if adr >= 0x800000i32 as cty::c_uint &&
                  adr <= 0xbfffffi32 as cty::c_uint {

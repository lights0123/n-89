...
#[no_mangle]
pub unsafe extern "C" fn ti92_mem_init() -> cty::c_int {
    // set mappers
    mem_get_byte_ptr =
        Some(ti92_get_byte as unsafe extern "C" fn(_: uint32_t) -> uint8_t);
    mem_get_word_ptr =
        Some(ti92_get_word as unsafe extern "C" fn(_: uint32_t) -> uint16_t);
    mem_get_long_ptr =
        Some(ti92_get_long as unsafe extern "C" fn(_: uint32_t) -> uint32_t);
    mem_put_byte_ptr =
        Some(ti92_put_byte as
                 unsafe extern "C" fn(_: uint32_t, _: uint8_t) -> ());
    mem_put_word_ptr =
        Some(ti92_put_word as
                 unsafe extern "C" fn(_: uint32_t, _: uint16_t) -> ());
    mem_put_long_ptr =
        Some(ti92_put_long as
                 unsafe extern "C" fn(_: uint32_t, _: uint32_t) -> ());
    mem_get_real_addr_ptr =
        Some(ti92_get_real_addr as
                 unsafe extern "C" fn(_: uint32_t) -> *mut uint8_t);
    return 0i32;
}
#[no_mangle]
pub unsafe extern "C" fn ti92_get_real_addr(mut adr: uint32_t)
 -> *mut uint8_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        return tihw.ram.offset((adr & (tihw.ram_size - 1i32) as cty::c_uint)
                                   as isize)
    } else {
        // PROM access
        if adr >= tihw.rom_base &&
               adr <=
                   tihw.rom_base.wrapping_add((2i32 * (1024i32 * 1024i32) -
                                                   1i32) as cty::c_uint) {
            return tihw.rom.offset((adr &
                                        (tihw.rom_size - 1i32) as
                                            cty::c_uint) as isize)
        } else {
            // memory-mapped I/O
...
#[no_mangle]
pub unsafe extern "C" fn ti92_get_long(mut adr: uint32_t) -> uint32_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        return ((((*tihw.ram.offset((adr &
                                         (tihw.ram_size - 1i32) as
        ...
        if adr >= tihw.rom_base &&
            ...
            return ((((*tihw.rom.offset((adr &
                          ...
                          *tihw.rom.offset((adr.wrapping_add(1i32 as
                                                ...
                                                (tihw.rom_size - 1i32) as
                                                    cty::c_uint) as isize) as
                              cty::c_int) as uint16_t as cty::c_int) <<
                        16i32 |
...
#[no_mangle]
pub unsafe extern "C" fn ti92_get_word(mut adr: uint32_t) -> uint16_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        return ((*tihw.ram.offset((adr &
                                       (tihw.ram_size - 1i32) as cty::c_uint)
...
#[no_mangle]
pub unsafe extern "C" fn ti92_get_byte(mut adr: uint32_t) -> uint8_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        return *tihw.ram.offset((adr & (tihw.ram_size - 1i32) as cty::c_uint)
                                    as isize)
...
#[no_mangle]
pub unsafe extern "C" fn ti92_put_long(mut adr: uint32_t, mut arg: uint32_t) {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        *tihw.ram.offset((adr & (tihw.ram_size - 1i32) as cty::c_uint) as
                             isize) =
            ((arg >> 16i32) as uint16_t as cty::c_int >> 8i32) as uint8_t;
        *tihw.ram.offset((adr.wrapping_add(1i32 as cty::c_uint) &
                              (tihw.ram_size - 1i32) as cty::c_uint) as
                             isize) =
        ...
        *tihw.ram.offset((adr.wrapping_add(2i32 as
            ...
            ((arg & 0xffffi32 as cty::c_uint) as uint16_t as cty::c_int &
                 0xffi32) as uint8_t
    } else if adr >= tihw.rom_base &&
                  adr <=
                      tihw.rom_base.wrapping_add((2i32 * (1024i32 * 1024i32) -
                                                      1i32) as cty::c_uint) {
        *tihw.rom.offset((adr & (tihw.rom_size - 1i32) as cty::c_uint) as
                             isize) =
            ((arg >> 16i32) as uint16_t as cty::c_int >> 8i32) as uint8_t;
        *tihw.rom.offset((adr.wrapping_add(1i32 as cty::c_uint) &
                              (tihw.rom_size - 1i32) as cty::c_uint) as
                             isize) =
            ((arg >> 16i32) as uint16_t as cty::c_int & 0xffi32) as uint8_t;
        *tihw.rom.offset((adr.wrapping_add(2i32 as cty::c_uint) &
                              (tihw.rom_size - 1i32) as cty::c_uint) as
                             isize) =
            ((arg & 0xffffi32 as cty::c_uint) as uint16_t as cty::c_int >>
                 8i32) as uint8_t;
...
#[no_mangle]
pub unsafe extern "C" fn ti92_put_word(mut adr: uint32_t, mut arg: uint16_t) {
    // PROM access
    // memory-mapped I/O
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn ti92_put_byte(mut adr: uint32_t, mut arg: uint8_t) {
    // PROM access
    // memory-mapped I/O
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {

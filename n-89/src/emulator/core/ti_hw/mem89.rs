...
#[no_mangle]
pub unsafe extern "C" fn ti89_mem_init() -> cty::c_int {
    // set mappers
    mem_get_byte_ptr =
        Some(ti89_get_byte as unsafe extern "C" fn(_: uint32_t) -> uint8_t);
    mem_get_word_ptr =
        Some(ti89_get_word as unsafe extern "C" fn(_: uint32_t) -> uint16_t);
    mem_get_long_ptr =
        Some(ti89_get_long as unsafe extern "C" fn(_: uint32_t) -> uint32_t);
    mem_put_byte_ptr =
        Some(ti89_put_byte as
                 unsafe extern "C" fn(_: uint32_t, _: uint8_t) -> ());
    mem_put_word_ptr =
        Some(ti89_put_word as
                 unsafe extern "C" fn(_: uint32_t, _: uint16_t) -> ());
    mem_put_long_ptr =
        Some(ti89_put_long as
                 unsafe extern "C" fn(_: uint32_t, _: uint32_t) -> ());
    mem_get_real_addr_ptr =
        Some(ti89_get_real_addr as
                 unsafe extern "C" fn(_: uint32_t) -> *mut uint8_t);
    return 0i32;
}
#[no_mangle]
pub unsafe extern "C" fn ti89_get_real_addr(mut adr: uint32_t)
 -> *mut uint8_t {
    // RAM access
...
#[no_mangle]
pub unsafe extern "C" fn ti89_get_long(mut adr: uint32_t) -> uint32_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        return ((((*tihw.ram.offset((adr &
...
#[no_mangle]
pub unsafe extern "C" fn ti89_get_word(mut adr: uint32_t) -> uint16_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        return ((*tihw.ram.offset((adr &
                                       (256i32 * 1024i32 - 1i32) as
...
#[no_mangle]
pub unsafe extern "C" fn ti89_get_byte(mut adr: uint32_t) -> uint8_t {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
        return *tihw.ram.offset((adr &
                                     (256i32 * 1024i32 - 1i32) as
...
#[no_mangle]
pub unsafe extern "C" fn ti89_put_long(mut adr: uint32_t, mut arg: uint32_t) {
    // RAM access
    if adr >= 0i32 as cty::c_uint && adr <= 0x1fffffi32 as cty::c_uint {
...
#[no_mangle]
pub unsafe extern "C" fn ti89_put_word(mut adr: uint32_t, mut arg: uint16_t) {
    // FLASH access
    // memory-mapped I/O
    // memory-mapped I/O (hw2)
    // RAM access
...
#[no_mangle]
pub unsafe extern "C" fn ti89_put_byte(mut adr: uint32_t, mut arg: uint8_t) {
    // FLASH access
    // memory-mapped I/O
    // memory-mapped I/O (hw2)
    // RAM access

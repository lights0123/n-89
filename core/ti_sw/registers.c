...
int ti68k_register_get_data(int n, uint32_t *val) {
	...
	if(n >= 0 && n < 8) *val = m68k_dreg(regs, n);

	if(m68k_dreg(regs, n) != old_d[n]) c = !0;
...
int ti68k_register_get_addr(int n, uint32_t *val) {
	...
	if(n >= 0 && n < 8) *val = m68k_areg(regs, n);

	if(m68k_areg(regs, n) != old_a[n]) c = !0;
...
int ti68k_register_get_sp(uint32_t *val) {
	...
	*val = m68k_areg(regs, 7);
	if(m68k_areg(regs, 7) != old_sp) c = !0;
...
const char *ti68k_register_get_flag(void) {
	...
	printf("T=%d S=%d I=%d | X=%d N=%d\nZ=%d V=%d C=%d\n", SR_get_T(regs.sr), SR_get_S(regs.sr),
	       SR_get_I(regs.sr), SR_get_X(regs.sr), SR_get_N(regs.sr), SR_get_Z(regs.sr),
...
int ti68k_register_get_flags(char *sf, char *uf) {
	...
	sprintf(uf, "X=%d N=%d \nZ=%d V=%d C=%d", /* %dSPC\n: SPC is important ! */
	        SR_get_X(regs.sr), SR_get_N(regs.sr), SR_get_Z(regs.sr), SR_get_V(regs.sr),
	        SR_get_C(regs.sr));

	if(strcmp(sf, old_sf) || strcmp(uf, old_uf)) c = !0;

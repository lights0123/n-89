...
void ti68k_kbd_set_key(int key, int active) {
	if(key == TIKEY_ALPHA) {
		...
		tihw.on_key = active;
		if(active) on_change = !0;
	} else {
		key_states[key] = active;
		if(active) key_change = !0;
	}
}
...
int ti68k_kbd_push_chars(const char *chars) {
	if(key_buffer) return FALSE;

	key_buffer = chars_to_keys(chars, tihw.calc_type == TI89 || tihw.calc_type == TI89t);

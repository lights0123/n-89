...
int hwp_fetch(uint32_t adr) {
	// protections (hw1)
	if(tihw.hw_type == HW1) {
		if(IN_BOUNDS2(0x390000 + tihw.archive_limit * 0x10000, adr, 0x3fffff)) // archive memory limit (hw1)
		{
...
uint8_t hwp_get_byte(uint32_t adr) {
	...
	{
		if(!tihw.protect && (tihw.hw_type == HW1)) bit_clr(tihw.archive_limit, 0);
	} else if(IN_BOUNDS(0x080000, adr, 0x0bffff)) // archive memory limit bit 1 (hw1)
	{
		if(!tihw.protect && (tihw.hw_type == HW1)) bit_clr(tihw.archive_limit, 1);
	} else if(IN_BOUNDS(0x0c0000, adr, 0x0fffff)) // archive memory limit bit 2 (hw1)
	{
		if(!tihw.protect && (tihw.hw_type == HW1)) bit_clr(tihw.archive_limit, 2);
...
void hwp_put_byte(uint32_t adr, uint8_t arg) {
	...
	if(IN_BOUNDS(0x040000, adr, 0x07ffff)) // archive memory limit bit 0 (hw1)
	{
		if(!tihw.protect && (tihw.hw_type == HW1)) bit_set(tihw.archive_limit, 0);
	} else if(IN_BOUNDS(0x080000, adr, 0x0bffff)) // archive memory limit bit 1 (hw1)
	{
		if(!tihw.protect && (tihw.hw_type == HW1)) bit_set(tihw.archive_limit, 1);
	} else if(IN_BOUNDS(0x0c0000, adr, 0x0fffff)) // archive memory limit bit 2 (hw1)
	{
		if(!tihw.protect && (tihw.hw_type == HW1)) bit_set(tihw.archive_limit, 2);
	} else if(IN_BOUNDS(0x180000, adr, 0x1bffff)) // screen power control
	{
		// tihw.on_off = !0;

...
int hw_io_init(void) {
	...
	if(tihw.hw_type > HW1) tihw.lcd_adr = 0x4c00;

	// set LCD on
	if(tihw.hw_type > HW1) tihw.io2[0x1d] = 2;
...
void io_put_byte(uint32_t addr, uint8_t arg) {
	...
	case 0x10: // -w <76543210> (hw1)
		...
		if(tihw.hw_type == HW1) tihw.lcd_adr = ((arg << 8) | tihw.io[0x11]) << 3;
		break;
	case 0x11: // -w <76543210> (hw1)
		// address of LCD memory divided by 8 (lsb)
		if(tihw.hw_type == HW1) tihw.lcd_adr = ((tihw.io[0x10] << 8) | arg) << 3;
		break;
...
uint8_t io_get_byte(uint32_t addr) {
	...
	case 0x00: // rw <76...2..>
		...
		if(tihw.calc_type == TI92) v = ((tihw.contrast & 1) << 5);
...
void io3_put_byte(uint32_t addr, uint8_t arg) {
	...
	case 0x5f: // ro & rw <......10>
		...
		if(!bit_tst(arg, 0)) {
			...
			tihw.rtc3_load.s =
			(tihw.io3[0x46] << 24) | (tihw.io3[0x47] << 16) | (tihw.io3[0x48] << 8) | tihw.io3[0x49];

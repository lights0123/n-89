...
void hw_update(void) {
	...
	if((tihw.hw_type >= HW2) && io2_bit_tst(0x1f, 2) && io2_bit_tst(0x1f, 1)) {
		...
		if((unsigned)(tmp.tv_sec - ref.tv_sec) * 500000u + ((unsigned)(tmp.tv_usec - ref.tv_usec) >> 1u) >= 4096000000u) {
	...
	// times consistent
	if(!(timer % 192) && tihw.hw_type >= HW2) tihw.io2[0x1d] ^= 0x80;
	...
	*/
	...
	if(!io_bit_tst(0x0c, 6)) {
		...
		if((io_bit_tst(0x0c, 3) && io_bit_tst(0x0d, 7)) || (io_bit_tst(0x0c, 2) && io_bit_tst(0x0d, 3)) ||
		   (io_bit_tst(0x0c, 1) && io_bit_tst(0x0d, 6)) || (io_bit_tst(0x0c, 0) && io_bit_tst(0x0d, 5))) {
			hw_m68k_irq(4);
		}
	}

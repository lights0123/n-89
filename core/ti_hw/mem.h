...
#define put_b(ptr, adr, mask, arg) \
	{ ptr[(adr) & (mask)] = (arg); }
#define put_w(ptr, adr, mask, arg)                            \
	{                                                         \
		put_b(ptr, adr, mask, (uint8_t)((arg) >> 8));         \
		put_b(ptr, (adr) + 1, mask, (uint8_t)((arg)&0x00ff)); \
	}
#define put_l(ptr, adr, mask, arg)                             \
	{                                                          \
		put_w(ptr, adr, mask, (uint16_t)((arg) >> 16));        \
		put_w(ptr, (adr) + 2, mask, (uint16_t)((arg)&0xffff)); \
	}

#define get_b(ptr, adr, mask) (ptr[(adr) & (mask)])
#define get_w(ptr, adr, mask) \
	((uint16_t)((get_b(ptr, adr, mask) << 8) | get_b(ptr, (adr) + 1, mask)))
#define get_l(ptr, adr, mask) \
	((uint32_t)((get_w(ptr, adr, mask) << 16) | get_w(ptr, (adr) + 2, mask)))

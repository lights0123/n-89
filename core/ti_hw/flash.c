...
int hw_flash_exit(void) {
	if(wsm.changed != NULL) free(wsm.changed);
...
uint8_t FlashReadByte(uint32_t addr) {
	if(wsm.write_phase == 0x90) {
		switch(addr & 0xffff) {
		case 0:
			return (tihw.calc_type == V200 || tihw.calc_type == TI89t) ? 0xb0 : 0x89; // manufacturer code
		case 1:
			return 0x00;
		case 2:
			return 0xb5; // device code
		case 3:
			return 0x00;
		default:
			return 0xff;
...
void FlashWriteByte(uint32_t addr, uint8_t v) {
	...
	if(wsm.write_ready) {
		if(rom[addr] != v) wsm.changed[addr >> 16] = !0;

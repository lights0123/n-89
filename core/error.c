...
int ti68k_error_get(int err_num, char **error_msg) {
	...
	case ERR_REVISION_MATCH:
		*error_msg = g_strdup(
		_("Can not open state image: revision changed. You have to recreate the state image."));
		break;

	case ERR_HEADER_MATCH:
		*error_msg = g_strdup(_("Can not open state image: state image header does not match ROM "
		                        "image header: have you changed/updated your ROM image?"));
		break;

	case ERR_STATE_MATCH:
		*error_msg = g_strdup(
		_("Can not open state image: this state image is not targetted for your current emulator "
		  "image (calculator model and/or OS version must match!). Choose another image before."));
		break;
	...
	default:
		*error_msg =
		g_strdup(_("Error code not found in the list.\nThis is a bug. Please report it."));
		return err_num;
	}

...
struct uae_input_device {
    char *name;
    int16_t eventid[MAX_INPUT_DEVICE_EVENTS][MAX_INPUT_SUB_EVENT];
    char *custom[MAX_INPUT_DEVICE_EVENTS][MAX_INPUT_SUB_EVENT];
    uint16_t flags[MAX_INPUT_DEVICE_EVENTS][MAX_INPUT_SUB_EVENT];
    int16_t extra[MAX_INPUT_DEVICE_EVENTS][MAX_INPUT_SIMULTANEOUS_KEYS];
    uint8_t enabled;
};
#endif
...
#if 0
    uint32_t z3fastmem_size;
    uint32_t fastmem_size;
    uint32_t chipmem_size;
    uint32_t bogomem_size;
    uint32_t a3000mem_size;
    uint32_t gfxmem_size;
...
STATIC_INLINE void fuzzy_memset_1 (void *p, uint32_t c, int offset, int len)
{
    uint32_t *p2 = (uint32_t *)((char *)p + offset);
    int a = len & 7;
    len >>= 3;
    switch (a) {
     case 7: p2--; goto l1;
...
#define fuzzy_memset_le32(p, c, o, l) \
	fuzzy_memset_le32_1((p), QUADRUPLIFY(c), (o) & ~3, ((l) + 7) >> 2)
STATIC_INLINE void fuzzy_memset_le32_1 (void *p, uint32_t c, int offset, int len)
{
    uint32_t *p2 = (uint32_t *)((char *)p + offset);

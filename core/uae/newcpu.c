...
static uint16_t last_op_for_exception_3;
/* PC at fault time */
static uintptr_t last_addr_for_exception_3;
/* Address that generated the exception */
static uintptr_t last_fault_for_exception_3;
...
static uint16_t opcodenums[65536];

static int compfn(const void *el1, const void *el2) {
	return instrcount[*(const uint16_t *)el1] < instrcount[*(const uint16_t *)el2];
}
...
void dump_counts(void) {
	...
	qsort(opcodenums, 65536, sizeof(uint16_t), compfn);
...
static unsigned long op_illg_1(uint32_t opcode);
...
static void build_cpufunctbl(void) {
	...
	(currprefs.cpu_level == 4 ?
	 ...
	 op_smalltbl_1_ff :
	 currprefs.cpu_level == 2 ? op_smalltbl_2_ff : currprefs.cpu_level == 1 ? op_smalltbl_3_ff : op_smalltbl_5_ff);
...
void init_m68k(void) {
	...
	if(currprefs.address_space_24 && currprefs.cpu_level > 1) write_log("EC");
...
int32_t ShowEA(FILE *f, int reg, amodes mode, wordsizes size, char *buf) {
	...
	case Apdi:
		...
		break;
	case Ad16:
		disp16 = get_iword_1(m68kpc_offset);
		m68kpc_offset += 2;
		addr = m68k_areg(regs, reg) + (int16_t)disp16;
		if(disp16 & 0x8000)
			sprintf(buffer, "(-$%X,A%d) [%s]", (-(int16_t)(disp16)) & 0xffff, reg, sym_addr((uint32_t)addr));
		else
			sprintf(buffer, "(+$%X,A%d) [%s]", disp16 & 0xffff, reg, sym_addr((uint32_t)addr));
		break;
	case Ad8r:
		dp = get_iword_1(m68kpc_offset);
		m68kpc_offset += 2;
		disp8 = dp & 0xFF;
		r = (dp & 0x7000) >> 12;
		dispreg = dp & 0x8000 ? m68k_areg(regs, r) : m68k_dreg(regs, r);
		if(!(dp & 0x800)) dispreg = (int32_t)(int16_t)(dispreg);
		dispreg <<= (dp >> 9) & 3;
		...
		if(dp & 0x100) {
			...
			if((dp & 0x30) == 0x20) {
				disp = (int32_t)(int16_t)get_iword_1(m68kpc_offset);
				m68kpc_offset += 2;
			}
			if((dp & 0x30) == 0x30) {
				disp = get_ilong_1(m68kpc_offset);
				m68kpc_offset += 4;
			}
			base += disp;
			...
			addr = base + outer;
			sprintf(buffer, "(%s%c%d.%c+%ld)+%ld [$%06lX]", name, dp & 0x8000 ? 'A' : 'D', (int)r,
			        dp & 0x800 ? 'L' : 'W', disp, outer, (unsigned long)addr);
		} else {
			addr = m68k_areg(regs, reg) + (int32_t)((int8_t)disp8) + dispreg;
			sprintf(buffer, "(A%d, %c%d.%c, $%02X) [$%06lX]", reg, dp & 0x8000 ? 'A' : 'D', (int)r,
			        dp & 0x800 ? 'L' : 'W', disp8, (unsigned long)addr);
		}
		break;
	case PC16:
		...
		addr += (int16_t)disp16;
		if(disp16 & 0x8000)
			sprintf(buffer, "(-$%X,PC) [%s]", (-(int16_t)(disp16)) & 0xffff, sym_addr((uint32_t)addr));
		else
			sprintf(buffer, "(+$%X,PC) [%s]", disp16 & 0xffff, sym_addr((uint32_t)addr));
		break;
	case PC8r:
		addr = m68k_getpc() + m68kpc_offset;
		dp = get_iword_1(m68kpc_offset);
		...
		if(dp & 0x100) {
			...
			addr = base + outer;
			sprintf(buffer, "(%s%c%d.%c+%ld)+%ld [%s]", name, dp & 0x8000 ? 'A' : 'D', (int)r,
			        dp & 0x800 ? 'L' : 'W', disp, outer, sym_addr((uint32_t)addr));
		} else {
			addr += (int32_t)((int8_t)disp8) + dispreg;
			sprintf(buffer, "(PC, %c%d.%c, $%02X) [%s]", dp & 0x8000 ? 'A' : 'D', (int)r,
			        dp & 0x800 ? 'L' : 'W', disp8, sym_addr((uint32_t)addr));
		}
		break;
	case absw:
		...
		break;
	case absl:
		sprintf(buffer, "%s", sym_addr((uint32_t)get_ilong_1(m68kpc_offset)));
		m68kpc_offset += 4;
		break;
	case imm:
		switch(size) {
...
static int verify_ea (int reg, amodes mode, wordsizes size, uint32_t *val)
{
    uint16_t dp;
    int8_t disp8;
    int16_t disp16;
	...
	*val = m68k_areg (regs, reg);
	...
	goto d8r_common;
     case absw:
	addr = (int32_t)(int16_t)get_iword_1 (m68kpc_offset);
	m68kpc_offset += 2;
	break;
     case absl:
	...
	return 1;
     case imm0:
	*val = (int32_t)(int8_t)get_iword_1 (m68kpc_offset);
	m68kpc_offset += 2;
	return 1;
     case imm1:
	*val = (int32_t)(int16_t)get_iword_1 (m68kpc_offset);
	m68kpc_offset += 2;
	return 1;
     case imm2:
	*val = get_ilong_1 (m68kpc_offset);
...
uint32_t get_disp_ea_020(uint32_t base, uint32_t dp) {
	int reg = (dp >> 12) & 15;
	int32_t regd = regs.regs[reg];
	if((dp & 0x800) == 0) regd = (int32_t)(int16_t)regd;
	regd <<= (dp >> 9) & 3;
	if(dp & 0x100) {
		int32_t outer = 0;
		if(dp & 0x80) base = 0;
		...
		if((dp & 0x30) == 0x20) base += (int32_t)(int16_t)next_iword();
		if((dp & 0x30) == 0x30) base += next_ilong();

		if((dp & 0x3) == 0x2) outer = (int32_t)(int16_t)next_iword();
		...
		return base + (int32_t)((int8_t)dp) + regd;
	}
}
...
void MakeSR(void) {
	...
	regs.sr = ((regs.t1 << 15) /*| (regs.t0 << 14)*/
	           | (regs.s << 13) /*| (regs.m << 12)*/ | (regs.intmask << 8) | (GET_XFLG << 4) |
...
static void exception_trace(int nr) {
	...
	if(regs.t1 /*&& !regs.t0*/) {
		 ...
		 * trapv or trap #x
		 */
		if(nr == 5 || nr == 6 || nr == 7 || (nr >= 32 && nr <= 47)) set_special(SPCFLAG_DOTRACE);
	}
...
void Exception_normal(int nr, uintptr_t oldpc) {
	...
	if(nr == 2 || nr == 3) {
		uint16_t mode = (sv ? 4 : 0) | (last_instructionaccess_for_exception_3 ? 2 : 1);
...
void Exception(int nr, uintptr_t oldpc) {
...
static uint32_t caar, cacr, itt0, itt1, dtt0, dtt1, tc, mmusr, urp, srp;

int m68k_move2c(int regno, uint32_t *regp) {
	if((currprefs.cpu_level == 1 && (regno & 0x7FF) > 1) ||
	   (currprefs.cpu_level < 4 && (regno & 0x7FF) > 2) || (currprefs.cpu_level == 4 && regno == 0x802)) {
		op_illg(0x4E7B);
		return 0;
	} else {
		switch(regno) {
		...
		case 4:
			itt0 = *regp & 0xffffe364;
			break;
		case 5:
			itt1 = *regp & 0xffffe364;
			break;
		case 6:
			dtt0 = *regp & 0xffffe364;
			break;
		case 7:
			dtt1 = *regp & 0xffffe364;
			break;
...
int m68k_movec2(int regno, uint32_t *regp) {
	if((currprefs.cpu_level == 1 && (regno & 0x7FF) > 1) ||
	   (currprefs.cpu_level < 4 && (regno & 0x7FF) > 2) || (currprefs.cpu_level == 4 && regno == 0x802)) {
		op_illg(0x4E7A);
		return 0;
...
STATIC_INLINE int div_unsigned(uint32_t src_hi, uint32_t src_lo, uint32_t div, uint32_t *quot, uint32_t *rem) {
	uint32_t q = 0, cbit = 0;
	int i;
...
void m68k_divl(uint32_t opcode, uint32_t src, uint16_t extra, uintptr_t oldpc) {
#if defined(uae_s64)
	if(src == 0) {
		Exception(5, oldpc);
		return;
	...
	if(extra & 0x800) {
		/* signed variant */
		uae_s64 a = (uae_s64)(int32_t)m68k_dreg(regs, (extra >> 12) & 7);
		uae_s64 quot, rem;
		...
		rem = a % (uae_s64)(int32_t)src;
		quot = a / (uae_s64)(int32_t)src;
		if((quot & UVAL64(0xffffffff80000000)) != 0 &&
		   (quot & UVAL64(0xffffffff80000000)) != UVAL64(0xffffffff80000000)) {
			...
			if(((int32_t)rem < 0) != ((uae_s64)a < 0)) rem = -rem;
			SET_VFLG(0);
			SET_CFLG(0);
			SET_ZFLG(((int32_t)quot) == 0);
			SET_NFLG(((int32_t)quot) < 0);
			m68k_dreg(regs, extra & 7) = rem;
			m68k_dreg(regs, (extra >> 12) & 7) = quot;
		}
	} else {
		/* unsigned */
		uae_u64 a = (uae_u64)(uint32_t)m68k_dreg(regs, (extra >> 12) & 7);
		uae_u64 quot, rem;
	...
	if(extra & 0x800) {
		/* signed variant */
		int32_t lo = (int32_t)m68k_dreg(regs, (extra >> 12) & 7);
		int32_t hi = lo < 0 ? -1 : 0;
		int32_t save_high;
		...
		if(extra & 0x400) {
			hi = (int32_t)m68k_dreg(regs, extra & 7);
		}
		save_high = hi;
		sign = (hi ^ src);
		if(hi < 0) {
		...
		if((int32_t)src < 0) src = -src;
		if(div_unsigned(hi, lo, src, &quot, &rem) || (sign & 0x80000000) ? quot > 0x80000000 : quot > 0x7fffffff) {
			SET_VFLG(1);
			SET_NFLG(1);
			SET_CFLG(0);
		} else {
			if(sign & 0x80000000) quot = -quot;
			if(((int32_t)rem < 0) != (save_high < 0)) rem = -rem;
			SET_VFLG(0);
			SET_CFLG(0);
			SET_ZFLG(((int32_t)quot) == 0);
			SET_NFLG(((int32_t)quot) < 0);
		...
		/* unsigned */
		uint32_t lo = (uint32_t)m68k_dreg(regs, (extra >> 12) & 7);
		uint32_t hi = 0;
		uint32_t quot, rem;

		if(extra & 0x400) {
			hi = (uint32_t)m68k_dreg(regs, extra & 7);
		}
...
STATIC_INLINE void mul_unsigned(uint32_t src1, uint32_t src2, uint32_t *dst_hi, uint32_t *dst_lo) {
	uint32_t r0 = (src1 & 0xffff) * (src2 & 0xffff);
	uint32_t r1 = ((src1 >> 16) & 0xffff) * (src2 & 0xffff);
	uint32_t r2 = (src1 & 0xffff) * ((src2 >> 16) & 0xffff);
	uint32_t r3 = ((src1 >> 16) & 0xffff) * ((src2 >> 16) & 0xffff);
	uint32_t lo;
...
void m68k_mull(uint32_t opcode, uint32_t src, uint16_t extra) {
#if defined(uae_s64)
	if(extra & 0x800) {
		/* signed variant */
		uae_s64 a = (uae_s64)(int32_t)m68k_dreg(regs, (extra >> 12) & 7);

		a *= (uae_s64)(int32_t)src;
		SET_VFLG(0);
		SET_CFLG(0);
		SET_ZFLG(a == 0);
		SET_NFLG(a < 0);
		...
		m68k_dreg(regs, (extra >> 12) & 7) = (uint32_t)a;
	} else {
		/* unsigned */
		uae_u64 a = (uae_u64)(uint32_t)m68k_dreg(regs, (extra >> 12) & 7);
	...
	if(extra & 0x800) {
		...
		src1 = (int32_t)src;
		src2 = (int32_t)m68k_dreg(regs, (extra >> 12) & 7);
		sign = (src1 ^ src2);
		if(src1 < 0) src1 = -src1;
		if(src2 < 0) src2 = -src2;
		mul_unsigned((uint32_t)src1, (uint32_t)src2, &dst_hi, &dst_lo);
		if(sign & 0x80000000) {
			dst_hi = ~dst_hi;
			dst_lo = -dst_lo;
			if(dst_lo == 0) dst_hi++;
		...
		SET_NFLG(((int32_t)dst_hi) < 0);
		if(extra & 0x400)
			m68k_dreg(regs, extra & 7) = dst_hi;
		else if((dst_hi != 0 || (dst_lo & 0x80000000) != 0) &&
		        ((dst_hi & 0xffffffff) != 0xffffffff || (dst_lo & 0x80000000) != 0x80000000)) {
			SET_VFLG(1);
		}
		...
		mul_unsigned(src, (uint32_t)m68k_dreg(regs, (extra >> 12) & 7), &dst_hi, &dst_lo);
...
#ifdef NO_GDB
static char *ccnames[] = { "T",  "F",  "HI", "LS", "CC", "CS", "NE", "EQ",
...
unsigned long op_illg(uint32_t opcode) {
#if 0
    uintptr_t pc = m68k_getpc ();
    
    if (cloanto_rom && (opcode & 0xF100) == 0x7100) {
	m68k_dreg (regs, (opcode >> 9) & 7) = (int8_t)(opcode & 0xFF);
	m68k_incpc (2);
	fill_prefetch_slow ();
	return 4;
    }
...
void mmu_op(uint32_t opcode, uint16_t extra) {
...
#if 0
static uintptr_t last_trace_ad = 0;
...
static void do_trace (void)
{
	...
	if (opcode == 0x4e72 		/* RTE */
		...
		&& !cctrue((opcode >> 8) & 0xf)
		&& (int16_t)m68k_dreg(regs, opcode & 7) != 0))
	{
...
static void exception2_handle (uintptr_t addr, uintptr_t fault)
{
...
static void m68k_verify (uintptr_t addr, uintptr_t *nextpc)
{
    uint32_t opcode, val;
    struct instr *dp;
...
int m68k_disasm(char *output, uintptr_t addr) {
	char buf[273];
	uintptr_t newpc = 0;
	...
	if(cpufunctbl[opcode] == op_illg_1 || (orig_opcode & 0xf000) == 0xa000 || (orig_opcode & 0xf000) == 0xf000) {
		if(!((orig_opcode >= 0xf800) && (orig_opcode <= 0xfff2))) opcode = 0x4AFC;
	}
	dp = table68k + opcode;
	for(lookup = lookuptab; lookup->mnemo != dp->mnemo; lookup++)
		;
	...
	if(dp->suse && dp->duse) strcat(output, ",");
	if(dp->duse) {
		newpc = m68k_getpc() + m68kpc_offset;
	...
	if(ccpt != 0) {
		...
		case 0xfff0: /* 6 byte bsr w/long word displacement */
			pm = get_ilong_1(m68kpc_offset);
			m68kpc_offset += 6 - 2;
			if(pm & 0x8000)
				sprintf(buffer, "FLINE bsr.l *-$%lX [%lX]", (-(signed long)(int32_t)pm) - 2,
				        pc + (signed long)(int32_t)pm + 2);
			else
		...
		case 0xfff1: /* 6 byte bra w/long word displacement */
			...
			m68kpc_offset += 6 - 2;
			if(pm & 0x8000)
				sprintf(buffer, "FLINE bra.l *-$%lX [%lX]", (-(signed long)(int32_t)pm) - 2,
				        pc + (signed long)(int32_t)pm + 2);
			else
				sprintf(buffer, "FLINE bra.l *+$%lX [%lX]", pm + 2, pc + pm + 2);
			break;
		case 0xfff2: /* 4 byte ROM CALL */
			pm = get_iword_1(m68kpc_offset);
			m68kpc_offset += 4 - 2;
		...
		case 0xffee: /* jmp __ld_entry_point_plus_0x8000+word */
			...
			m68kpc_offset += 4 - 2;
			{
				...
				if(handle > 0)
					...
					addr = 0;
				sprintf(buffer, "FLINE jmp.w *+$%lX [%lX]", (signed long)(signed short)pm + 0x8000,
				        addr + (signed long)(signed short)pm + 0x8000);
			}
			break;
		case 0xffef: /* jsr __ld_entry_point_plus_0x8000+word */
			...
			m68kpc_offset += 4 - 2;
			{
				...
				if(handle > 0)
					...
					addr = 0;
				sprintf(buffer, "FLINE jsr.w *+$%lX [%lX]", (signed long)(signed short)pm + 0x8000,
				        addr + (signed long)(signed short)pm + 0x8000);
			}
			break;
...
#if 0
void m68k_dumpstate (FILE *f, uintptr_t *nextpc)
{
    int i;
    for (i = 0; i < 8; i++){
...
uint8_t *restore_cpu (uint8_t *src)
{
    int i,model,flags;
    uint32_t l;
...
uint8_t *save_cpu (int *len, uint8_t *dstptr)
{
    uint8_t *dstbak,*dst;
    int model,i;
...
static void exception3f(uint32_t opcode, uintptr_t addr, uintptr_t fault, int writeaccess, int instructionaccess) {
	last_addr_for_exception_3 = addr;
	last_fault_for_exception_3 = fault;
	last_op_for_exception_3 = opcode;
...
void exception3(uint32_t opcode, uintptr_t addr, uintptr_t fault) {
	exception3f(opcode, addr, fault, 0, 0);
}

void exception3i(uint32_t opcode, uintptr_t addr, uintptr_t fault) {
	exception3f(opcode, addr, fault, 0, 1);
}

#if 0
void exception2 (uintptr_t addr, uintptr_t fault)
{
...
void cpureset (void)
{
    ...
    if ((ins & ~7) == 0x4ed0) {
	int reg = ins & 7;
	uint32_t addr = m68k_areg (regs, reg);

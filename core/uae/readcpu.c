...
struct mnemolookup lookuptab[] = {
	{ i_MOVEQ, "MOVEQ" },     { i_ADDQ, "ADDQ" },       { i_SUBQ, "SUBQ" },
	{ i_CMPI, "CMPI" },       { i_ADDI, "ADDI" },       { i_SUBI, "SUBI" },
	{ i_ORI, "ORI" },         { i_ANDI, "ANDI" },       { i_EORI, "EORI" },
...
out2:
	...
	for(variants = 0; variants < (1 << id.n_variable); variants++) {
		...
		if(bitval[bitz] == 3 || bitval[bitC] == 1) continue;
		if(bitcnt[bitI] && (bitval[bitI] == 0x00 || bitval[bitI] == 0xff)) continue;
		...
		if(opcstr[pos] == 0) goto endofline;
		...
		case '#':
			...
			case 'i':
				srcmode = immi;
				srcreg = (int32_t)(int8_t)bitval[biti];
				if(CPU_EMU_SIZE < 4) {
		...
		case 'd':
			...
			if(CPU_EMU_SIZE < 2 && (srcmode == Areg || srcmode == Dreg || srcmode == Aind || srcmode == Ad16 ||
			                        srcmode == Ad8r || srcmode == Aipi || srcmode == Apdi)) {
				srcgather = 1;
				srcpos = bitpos[bitD];
			}
			if(opcstr[pos] == '[') {
			...
			if(srcmode == imm || srcmode == PC16 || srcmode == PC8r) goto nomatch;
			break;
		case 's':
			srcreg = bitval[bitS];
		...
		if(srcmode != Areg && srcmode != Dreg && srcmode != Aind && srcmode != Ad16 &&
		   srcmode != Ad8r && srcmode != Aipi && srcmode != Apdi && srcmode != immi) {
			srcgather = 0;
		}
		if(srcmode == Areg && sz == sz_byte) goto nomatch;
		...
		case '#':
			...
			case 'i':
				destmode = immi;
				destreg = (int32_t)(int8_t)bitval[biti];
				break;
			case 'j':
				destmode = immi;
				destreg = bitval[bitj];
		...
		case 'd':
			...
			if(CPU_EMU_SIZE < 1 && (destmode == Areg || destmode == Dreg || destmode == Aind || destmode == Ad16 ||
			                        destmode == Ad8r || destmode == Aipi || destmode == Apdi)) {
				dstgather = 1;
				dstpos = bitpos[bitD];
			}
			...
			/* Some addressing modes are invalid as destination */
			if(destmode == imm || destmode == PC16 || destmode == PC8r) goto nomatch;
			break;
		case 's':
			destreg = bitval[bitS];
		...
		/* safety check - might have changed */
		if(destmode != Areg && destmode != Dreg && destmode != Aind && destmode != Ad16 &&
		   destmode != Ad8r && destmode != Aipi && destmode != Apdi) {
			dstgather = 0;
		}

		if(destmode == Areg && sz == sz_byte) goto nomatch;
#if 0
...
void read_table68k(void) {
	...
	if(table68k == NULL) table68k = (struct instr *)xmalloc(65536 * sizeof(struct instr));
	for(i = 0; i < 65536; i++) {
...
static void handle_merges(long int opcode) {
	...
	for(srcreg = 0; srcreg < sbitdst; srcreg++) {
		for(dstreg = 0; dstreg < dstend; dstreg++) {
			...
			if(table68k[opcode].suse && (table68k[opcode].spos != table68k[code].spos ||
			                             table68k[opcode].smode != table68k[code].smode ||
			                             table68k[opcode].stype != table68k[code].stype)) {
				mismatch++;
				continue;
			}
			if(table68k[opcode].duse && (table68k[opcode].dpos != table68k[code].dpos ||
			                             table68k[opcode].dmode != table68k[code].dmode)) {
				mismatch++;
				continue;
			}

			if(code != opcode) table68k[code].handler = opcode;
		}
	}
}
...
void do_merges(void) {
	...
	for(opcode = 0; opcode < 65536; opcode++) {
		if(table68k[opcode].handler != -1 || table68k[opcode].mnemo == i_ILLG) continue;
		nr++;
